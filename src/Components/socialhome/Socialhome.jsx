...
const Socialhome = () => {
  ...
  const myposts = useSelector(
    (state) => state.social?.myPosts?.postToShow?.posts
  );
  const userimage = useSelector(
    (state) => state.social?.myPosts?.postToShow?.avatar
  );
  const allusers = useSelector((state) => state?.users?.allusers?.users);
  const loggedInUser = useSelector(
    (state) => state.users?.userAuth?.userInfo?.user
  );
        ...
        <div className="mobile-parent-div">
          <div className="mobile-navbar-div">
            <button
              ...
              className="navbar-mobile-crossfit-button"
            >
              <p className="cross-mobile-para">
                cross<span className="fit-mobile-para">fit</span>
              </p>
            </button>
            <button
              className="navbar-mobile-hamburger-button"
          ...
          <div
            className={
              clicked ? "active-mobile-user-info" : "unactive-mobile-user-info"
            }
          >
            <div className="socialhome-left-profile-div">
              <img
                className="socialhome-left-img"
                src={loggedInUser?.avatar}
                alt="user-avavtar"
              />
              <h2 className="socialhome-left-name">{loggedInUser?.name}</h2>
              <h4 className="socialhome-left-desc">
                Artist,Trainer,Web Developer
              </h4>
              <h1 className="socialhome-left-no-post">
                {loggedInUser?.posts?.length} Posts
              </h1>
              <div className="socialhome-left-BUTTON-div">
                <button
                  onClick={mypostclickbtn}
                  className="socialhome-left-BUTTON"
                >
                  MY POSTS
                </button>
                <button
                  onClick={allpostclickbtn}
                  className="socialhome-left-BUTTON"
                >
                  ALL POSTS
                </button>
                <button
                  onClick={newpostbuttonClicked}
                  className="socialhome-left-BUTTON"
                >
                  NEW POST
                </button>
          ...
          <div>
            {loading ? (
              ...
              myposts?.map((item) => (
                ...
                <CardPost
                  name={item.user.name}
                  captionheading={item.captionHeading}
                  captiondesc={item.captionDescription}
        ...
        <div className="socialhome-main-div">
          <div className="socialhome-left-div">
            <div className="socialhome-left-heading-div">
              <p
                onClick={movetolandingpage}
                className="socialhome-left-heading-para-cross"
              >
                cross
                <span className="socialhome-left-heading-para-fit">fit</span>
              </p>
            </div>
            <div className="socialhome-left-profile-div">
              <img
